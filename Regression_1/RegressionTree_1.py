...
Y_test_arr = []
X_test = []
with open('input_train1.csv', 'rb') as csvfile:
	...
	c = row[5].replace(u'\xa0', ' ').split(' ')

	if(b[0]== '' or b[0]== ''):
		 b[0]=999
	if(a[0]== 'N/A' or a[0]== ''):
		 a[0]=0
	X_arr.append( [int(a[0]),float(b[0])])
	
with open('label_train1.csv', 'rb') as csvfile:
     spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
     for row in spamreader:
	b = row[1].split(',')
	b1 =row[3].split(',')
...
with open('label_test1.csv', 'rb') as csvfile:
     spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
     for row in spamreader:
	b = row[1].split(',')
	b1 =row[3].split(',')
...
with open('input_test1.csv', 'rb') as csvfile:
     spamreader = csv.reader(csvfile, delimiter=',', quotechar=' ')
     for row in spamreader:
	row1 =  (row[:1]) if (len(row)> 1) else row
	#print row[0][0]
...
regr_1 = DecisionTreeRegressor(min_samples_split=0.50)
...
ax.text(50, 45, ''.join(['MSE for (min_samples_split=0.50) : ',str(mse1)]), style='italic',
...
ax.text(25, 45, ''.join(['R2 for (min_samples_split=0.50) : ',str(R2_1)]), style='italic',
        bbox={'facecolor':'red', 'alpha':0.1, 'pad':10})
...
plt.plot(X_test, y_1, color="cornflowerblue",label="min_samples_split=0.50", linewidth=2)
plt.plot(X_test, y_2, color="yellowgreen", label="max_depth=5", linewidth=2)
plt.plot(X_test, y_3, color="red", label="Max_depth=12", linewidth=3)
plt.xlabel("data (Humidity,Temperature)")
plt.ylabel("target(Humidity,Temperature)	")
